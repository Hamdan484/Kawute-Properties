import { BedDouble, Bath, MapPin, Heart, ArrowRight } from "lucide-react";
import { useNavigate } from "react-router-dom";
import { useState, useEffect } from "react";

const PropertyCard = ({
  property,
  onLikeChange,
  hideHeart = false,
}) => {
  const navigate = useNavigate();
  const [isLiked, setIsLiked] = useState(false);

  // Check if this property is already liked
  useEffect(() => {
    const likedIds = JSON.parse(
      localStorage.getItem("likedProperties") || "[]"
    );

    setIsLiked(
      likedIds.includes(property.property_id)
    );
  }, [property.property_id]);

  // Like / Unlike property
  const handleLike = (e) => {
    e.stopPropagation();

    const likedIds = JSON.parse(
      localStorage.getItem("likedProperties") || "[]"
    );

    let newLikedState;

    if (isLiked) {
      // Remove property from liked properties
      const updatedIds = likedIds.filter(
        (id) => id !== property.property_id
      );

      localStorage.setItem(
        "likedProperties",
        JSON.stringify(updatedIds)
      );

      newLikedState = false;
    } else {
      // Add property to liked properties
      if (!likedIds.includes(property.property_id)) {
        likedIds.push(property.property_id);

        localStorage.setItem(
          "likedProperties",
          JSON.stringify(likedIds)
        );
      }

      newLikedState = true;
    }

    setIsLiked(newLikedState);

    // Notify other components
    window.dispatchEvent(
      new CustomEvent("likesUpdated", {
        detail: {
          propertyId: property.property_id,
          isLiked: newLikedState,
        },
      })
    );

    // Notify parent component
    if (onLikeChange) {
      onLikeChange(
        property.property_id,
        newLikedState
      );
    }
  };

  const {
    title,
    price,
    city,
    area,
    bed_rooms,
    bath_rooms,
    appartment_type,
    availability,
    image_url,
  } = property || {};

  const formattedPrice = Number(price)
    ? `$ ${Number(price).toLocaleString()}`
    : "Price unavailable";

  const imageSrc =
    image_url ||
    "https://images.unsplash.com/photo-1505693416388-ac5ce068fe85?auto=format&fit=crop&w=1200&q=80";

  return (
    <div className="group mt-2 w-full max-w-sm overflow-hidden rounded-3xl bg-white shadow-md transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl">

      {/* Image */}
      <div className="relative overflow-hidden">

        <img
          src={imageSrc}
          alt={title || "Property"}
          className="h-64 w-full object-cover transition-transform duration-500 group-hover:scale-110"
        />

        {/* Apartment Type */}
        <span className="absolute left-4 top-4 rounded-full bg-black px-4 py-2 text-sm font-semibold text-white">
          {appartment_type || "Property"}
        </span>

        {/* Like Button */}
        {!hideHeart && (
          <button
            onClick={handleLike}
            className="absolute right-4 top-4 rounded-full bg-black p-2 shadow-lg transition hover:bg-white"
          >
            <Heart
              size={18}
              className={
                isLiked
                  ? "fill-red-500 text-red-500"
                  : "text-white"
              }
            />
          </button>
        )}

      </div>

      {/* Content */}
      <div className="p-6">

        {/* Price and Availability */}
        <div className="mb-3 flex items-center justify-between">

          <h2 className="text-2xl font-bold text-black">
            {formattedPrice}
          </h2>

          <span className="rounded-full border border-black px-3 py-1 text-xs font-medium text-black">
            {availability || "Available"}
          </span>

        </div>

        {/* Title */}
        <h3 className="mb-2 text-xl font-bold text-black">
          {title || "Untitled Property"}
        </h3>

        {/* Location */}
        <div className="mb-5 flex items-center gap-2 text-gray-600">
          <MapPin size={18} />

          <span>
            {city || area || "Unknown Location"}
          </span>
        </div>

        {/* Features */}
        <div className="mb-6 flex items-center justify-between border-y py-4 text-black">

          <div className="flex items-center gap-2">
            <BedDouble size={18} />

            <span className="text-sm font-medium">
              {bed_rooms ?? "--"} Beds
            </span>
          </div>

          <div className="flex items-center gap-2">
            <Bath size={18} />

            <span className="text-sm font-medium">
              {bath_rooms ?? "--"} Baths
            </span>
          </div>

          <span className="text-sm font-medium">
            {area || "--"}
          </span>

        </div>

        {/* View Details */}
        <button
          onClick={() => {
            navigate(
              `/property/${property.property_id}`
            );
          }}
          className="flex w-full items-center justify-center gap-2 rounded-xl bg-black px-5 py-3 font-semibold text-white transition hover:bg-gray-800"
        >
          View Details

          <ArrowRight size={18} />
        </button>

      </div>
    </div>
  );
};

export default PropertyCard;